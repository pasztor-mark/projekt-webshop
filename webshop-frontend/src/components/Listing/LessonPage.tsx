...
import { getCookie, Lesson } from "../../../../shared/types"
...
export default function LessonPage() {
   ...
   return (
        <>
        {
            ...
            <div>
                <time className="flex items-center gap-4"><FaClockRotateLeft/>{new Date(lesson!.startTime).toLocaleString()}</time>
                <time className="flex items-center gap-4"><FaCheckDouble/>{new Date(lesson!.endTime).toLocaleString()}</time>
            </div>
        </div>
